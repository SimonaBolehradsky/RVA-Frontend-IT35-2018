...
export class ObrazovanjeDialogComponent implements OnInit {
  ...
  public addObrazovanje():void{
    this.obrazovanjeService.addObrazovanje(this.data).subscribe(()=>{
      this.snackBar.open('Uspesno dodato obrazovanje: '+ this.data.naziv, 'OK', {duration:2500})
    }),
    (error:Error)=> {
      console.log(console.error.name + ' '+ error.message);
      this.snackBar.open('Doslo je do greske prilikom dodavanja novog obrazovanja: ', 'Zatvori', {duration:2500})
  ...
  public updateObrazovanje():void {
    this.obrazovanjeService.updateObrazovanje(this.data).subscribe(()=>{
      this.snackBar.open('Uspesno modifikovano obrazovanje: '+ this.data.naziv, 'OK', {duration:2500})
    }),
    (error:Error)=> {
      console.log(console.error.name + ' '+ error.message);
      this.snackBar.open('Doslo je do greske prilikom modifikacije postojeceg obrazovanja: ', 'Zatvori', {duration:2500})
...
public deleteObrazovanje():void {
  this.obrazovanjeService.deleteObrazovanje(this.data.id).subscribe(()=>{
    this.snackBar.open('Uspesno obrisano obrazovanje: '+ this.data.naziv, 'OK', {duration:2500})
  }),
  (error:Error)=> {
    console.log(console.error.name + ' '+ error.message);
    this.snackBar.open('Doslo je do greske prilikom brisanja obrazovanja: ', 'Zatvori', {duration:2500})
